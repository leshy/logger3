...
(function(){
  var ref$, map, fold1, keys, values, first, flatten, h, net, Backbone, subscriptionMan, _, colors, os, util, throwError, ignoreError, callable, parseTags, Logger, parseArray, Console, Udp, Tcp, tcpServer, Fluent, slice$ = [].slice;
  ...
  Fluent = exports.Fluent = Backbone.Model.extend4000({
    ...
    initialize: function(settings){
      ...
      return this.logger.configure(os.hostname() + '.n.' + (this.settings.name || "unnamed"), {
        host: this.settings.host,
        port: this.settings.port
      });
    },
    log: function(logEvent){
      return this.logger.emit(keys(logEvent.tags).join('.'), _.extend({}, this.settings.extendPacket || {}, logEvent.data));
    }
  });
  function compose$() {
